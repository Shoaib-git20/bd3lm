...
def apply_tp_fsdp_with_sp(model, device_mesh):
    """
    Applies Tensor Parallelism + Sequence Parallelism + FSDP.
    """
    tp_mesh = device_mesh["tp"]
    dp_mesh = device_mesh["dp"]

    # 1. Define the Sequence Parallel Plan
    # We use Shard(1) because the sequence dimension is dim 1 in [Batch, Seq, Hidden]
    
    tp_plan = {
        # EMBEDDINGS
        # Input indices are Replicated. 
        # We set output_layouts=Shard(1) to start the Sequence Parallel flow immediately.
        ...
        # OUTPUT LAYER
        # Input is Shard(1) from the last block.
        # We Gather (Replicate) the output so the Loss function sees the full tensor.
        "backbone.output_layer.linear": ColwiseParallel(
            input_layouts=Shard(1),
            output_layouts=Replicate() 
        ),
        
        # Final Norm (runs on sharded sequence)
    ...
    for i in range(n_blocks):
        ...
        tp_plan.update({
            # --- Norms ---
            # These run efficiently on just the local slice of the sequence
            f"{block_prefix}.norm1": SequenceParallel(),
            f"{block_prefix}.norm2": SequenceParallel(),
            
            # --- Attention ---
            # QKV: Input is Shard(1). It internally All-Gathers before matmul.
            f"{block_prefix}.attn_qkv": ColwiseParallel(
                input_layouts=Shard(1)
            ),
            # OUT: Output is Reduce-Scattered back to Shard(1).
            f"{block_prefix}.attn_out": RowwiseParallel(
                output_layouts=Shard(1)
            ),
            
            # --- MLP ---
            # FC1: Input is Shard(1). All-Gather -> Compute.
            #f"{block_prefix}.mlp.0": ColwiseParallel(
            #    input_layouts=Shard(1)
            #),
            ## FC2: Output is Reduce-Scattered back to Shard(1).
            #f"{block_prefix}.mlp.2": RowwiseParallel(
            #    output_layouts=Shard(1)
            #),
        })
    ...
    if is_main_process():
        ...
        # DEBUG SNIPPET
        print("\n--- DEBUGGING LAYER TYPES ---")
        block_0 = model.backbone.blocks[0]
        print(f"Type of mlp:   {type(block_0.mlp)}")   # Should be nn.Sequential
        print(f"Type of mlp.0: {type(block_0.mlp[0])}") # Should be nn.Linear
        print(f"Content of mlp: {block_0.mlp}")
        print("-----------------------------\n")

    # 2. Apply Tensor/Sequence Parallelism
...
def setup_distributed_model(model, logger, config, local_rank):
    ...
    if strategy_name == 'ddp':
        ...
        model = apply_tp_fsdp_with_sp(model, mesh_2d)
