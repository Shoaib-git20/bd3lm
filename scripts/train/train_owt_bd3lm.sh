...
mpirun -np 1 python -u main.py loader.global_batch_size=16 loader.eval_global_batch_size=16 loader.batch_size=16 loader.eval_batch_size=16 \
    model=small algo=bd3lm algo.clip_search_widths=[0.5] \
    data=openwebtext-split data.insert_train_special=False data.insert_valid_special=False data.insert_valid_eos=False \
    model.length=1024 block_size=${BLOCK_SIZE} wandb.name=bd3lm-owt-block_size${BLOCK_SIZE} \
